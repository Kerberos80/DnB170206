...
$(document).ready(function() {
	...
	$(".joinTable #idCheck").click(function() {		
		...
		$.ajax({
			...
			error : function(x, e) { // 통신 실패시 콜백함수
	...
	function passwordValidate(currentPass) {
		...
		var passwordRegExpr = /^(?=.*[a-zA-Z])(?=.*[\d])((?=.*[_])|(?=.*[\W])).{6,15}$/; // \d(숫자), \W(_를 제외한 특수문자)		
		
		if ( !passwordRegExpr.test(currentPass) ) {			
			alert("비밀번호는 [6~15 자리] [영문자+숫자+특수문자] 이어야 합니다!");
			$(".joinTable #password").focus();
			return false;
		}
